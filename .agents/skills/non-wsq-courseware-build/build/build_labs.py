...
for lab in labs:
    ...
    L.append(f"**Course:** {C.TITLE}<br>")
    L.append(f"**Course Code:** {C.COURSE_CODE}<br>")
    L.append(f"**Version:** {C.VERSION} ({C.VERSION_DATE})<br>")
    L.append(f"**Topic {lab['topic']}:** {topic.get('title','')}<br>")
    L.append(f"**Maps to:** {lab['objective']}<br>")
    L.append(f"**Duration:** {lab.get('duration','—')} minutes<br>")
...
R = [f"# {C.TITLE} — Hands-On Labs\n",
     f"{len(labs)} labs across {len(C.TOPICS)} topics · {C.DAYS} day{'s' if C.DAYS != 1 else ''} · "
