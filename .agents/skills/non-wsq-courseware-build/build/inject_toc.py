...
def main():
    ...
    # Static TOC rows use compact 2 pt spacing, allowing the current 37 entries
    # plus heading to remain on one page without crowding.
    LINES_PER_PAGE = 40
    ...
    #    The builder places a dedicated page-break paragraph after the field.
    #    When a static TOC fills the preceding page, that paragraph can itself
    #    spill to the next page and create a blank page before the body. Replace
    #    it with page_break_before on the first body paragraph instead.
    anchor = placeholder._p
    break_para = anchor.getnext()
    body_para = break_para.getnext() if break_para is not None else None
    break_xml = break_para.xml if break_para is not None else ""
    has_page_break = break_para is not None and 'w:type="page"' in break_xml
    ...
    if has_page_break and body_para is not None and body_para.tag == qn("w:p"):
        from docx.text.paragraph import Paragraph
        Paragraph(body_para, placeholder._parent).paragraph_format.page_break_before = True
        parent.remove(break_para)
