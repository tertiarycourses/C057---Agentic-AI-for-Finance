...
def test_slide(act_title,text,kicker):
    s=head(slide(),act_title,kicker,TEAL)
    rect(s,Inches(0.85),Inches(2.3),Inches(11.7),Inches(3.75),DK_PANEL if THEME["dark"] else RGBColor(0xE8,0xF7,0xEE))
    txt(s,Inches(1.2),Inches(2.55),Inches(11),Inches(0.5),[[("✅  Test it",20,DK_GREEN if THEME["dark"] else RGBColor(0x12,0x7A,0x3E),True)]])
    # Verification criteria can be materially longer than ordinary slide copy.
    # Give the box enough vertical room and scale only this paragraph so every
    # criterion remains inside the green panel in PowerPoint and PDF renders.
    fs = 17 if len(text) <= 460 else 15 if len(text) <= 650 else 14
    txt(s,Inches(1.2),Inches(3.2),Inches(11),Inches(2.6),[[(text,fs,_ink(),False)]])
    footer(s); return s
...
tile_grid("Course Trainer Profiles",getattr(C,"TRAINER_TEAM",[]),
          kicker=f"TRAINER PROFILE · {C.COURSE_CODE}",cols=2,size=15,accent=BLUE)
...
txt(_dl,Inches(0.85),Inches(6.5),Inches(11.6),Inches(0.4),
    [[(f"Course page:  {getattr(C,'COURSE_URL','www.tertiarycourses.com.sg')}",12,_acc(BLUE),True)]],align=PP_ALIGN.CENTER)
...
if _OVERVIEW:
    ...
    if _OVERVIEW.get("pillars"):
        _pillars = _OVERVIEW["pillars"]
        if len(_pillars) <= 3:
            cards3(_OVERVIEW.get("pillars_title","What You'll Build"),
                   [(CARD_COLORS[i%3],t,b) for i,(t,b) in enumerate(_pillars)],
                   kicker="COURSE ROADMAP")
        else:
            tile_grid(_OVERVIEW.get("pillars_title","What You'll Build"),
                      [(title, " · ".join(body) if isinstance(body, list) else body)
                       for title, body in _pillars],
                      kicker="COURSE ROADMAP", cols=2, size=14, accent=BLUE)
